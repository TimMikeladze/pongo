...
export function LatencyHeatmap({ results, height = 120 }: LatencyHeatmapProps) {
  ...
  const heatmapData = useMemo(() => {
    const hourlyData: { [key: string]: { avg: number; max: number; count: number } } = {}
      ...
      <div className="flex flex-col gap-2" style={{ height }}>
        ...
        <div className="flex items-center gap-3 text-[9px] font-mono text-muted-foreground mt-2">
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded" style={{ backgroundColor: isDark ? "#4ade80" : "#16a34a" }} />
            <span>{"<200ms"}</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded" style={{ backgroundColor: isDark ? "#facc15" : "#ca8a04" }} />
            <span>200-500ms</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded" style={{ backgroundColor: isDark ? "#ef4444" : "#dc2626" }} />
            <span>{">500ms"}</span>
          </div>
        </div>
      </div>
