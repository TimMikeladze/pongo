...
// ============================================
// Alert overrides (operational controls)
// ============================================

export const getAlertOverrides = cache(
  async (): Promise<Record<string, AlertOverride>> => {
    const db = await getDb();
    const rows = (await db.select().from(alertOverrides)) as AlertOverride[];

    const map: Record<string, AlertOverride> = {};
...
export async function silenceAlert(
  ...
  if (existing) {
    await db
      .update(alertOverrides)
      .set({ silencedUntil: until, updatedAt: new Date() })
      .where(eq(alertOverrides.alertId, alertId));
  } else {
    await db.insert(alertOverrides).values({ alertId, silencedUntil: until });
  }
}

export async function unsilenceAlert(alertId: string): Promise<void> {
  const db = await getDb();
  await db
    .update(alertOverrides)
    .set({ silencedUntil: null, updatedAt: new Date() })
    .where(eq(alertOverrides.alertId, alertId));
}
...
export async function disableAlert(alertId: string): Promise<void> {
  ...
  if (existing) {
    ...
    await db.insert(alertOverrides).values({ alertId, disabled: true });
  }
}
