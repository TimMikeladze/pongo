...
export function StatsCard({ title, value, description, trend, className }: StatsCardProps) {
  ...
  return (
    <div className={cn("rounded border border-border bg-card", isDense ? "p-2" : "p-4", className)}>
      <div
        className={cn(
      ...
      {description && (
        <p className={cn("text-muted-foreground", isDense ? "mt-0.5 text-[8px]" : "mt-1 text-[10px]")}>{description}</p>
      )}
    </div>
  )
}
